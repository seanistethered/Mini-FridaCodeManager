...
func nextLine(in textView: UITextView) -> String? {
    guard let text = textView.text, let selectedRange = textView.selectedTextRange else {
        return nil
    }
    
    let caretPosition = selectedRange.start
    let offset = textView.offset(from: textView.beginningOfDocument, to: caretPosition)
    
    let textAfterCaret = text[text.index(text.startIndex, offsetBy: offset)...]
    
    if let firstNewlineIndex = textAfterCaret.firstIndex(of: "\n") {
        let nextLineStartIndex = text.index(after: firstNewlineIndex)
        let nextLine = text[nextLineStartIndex...].prefix { $0 != "\n" }
